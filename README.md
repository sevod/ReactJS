...
shallowEquality где то на 25-35 мин, это что? относится к store

selector в mstp на 36 мин это что? что то конкретное или общее понятие?

reselect библиотека упоминается на 38 мин, в прошедшем курсе не было

с 39 мин о nodeJs. Для использования во вспомогательных целях.

с 41-42 мин о компонентах.

на 43 мин упоминание функции map, акцент на key в мап

FLUX однонаправленный поток данных (или как?)


reducer (47 мин) это функция которая изменяет state (состояние). Она использует action's для изменения state. Для этого обычно используется инструмент Redux.
Или Редюсер (reducer) — это чистая функция, которая принимает предыдущее состояние и экшен (state и action) и возвращает следующее состояние (новую версию предыдущего).
А зачем тут dispath

redux-dux это концепция... а зачем?

Презентационная, функциональная компоненнта (50мин)

Контейнерная компонента (51) мин

Контекст api (51.34), контекст - это концепция ... не раскрыто в курсе, очень поверхностно. Технология будущего/настоящего? сторов может быть несколько?
React.createContext(defaultValue);
<MyContext.Provider value={}>
Дочерние компоненты становятся Consumer'ами
<MyContext.Consumer>
    {value => .......}
</MyContext.Consumer> 

Чистая функция (54.57)   

Что такое сайд эффекты???

Взаимодействие с сервером (58 мин.)
Интерфейс сервера, endPoint'ы, URL'ы, сервер API (Server REST API), CRUD (get, post, put, delete)

axios - библиотека для запроса данных с сервера. Возвращается промис (promis). Может быть пэндинг (вроде это промис,ожидание ), резолвед и режектед.
.then() - сюда придет асинхронный ответ 

Методы жизненного цикла (1.01.35) у классового компонента.

Если классовый компонент - реакт создает объект, если функциональная вызывает функцию.

Методы жизненного цикла (1.02.35) у классового компонента.  https://ru.reactjs.org/docs/state-and-lifecycle.html
render, componentDidMount, componentWillUnmount
this.setState() - стэйт меняем только так, не напрямую. Исключение конструктор.

whithRouter (1.08.03) -hoc для получения URL какая библиотека?

Логин (1.09), куки

UI-BLL-DAL-server
Thunk (1.11.51)  принимает в себя диспатч, делает асинхронную операцию и по окончанию них множественные операции, диспатчи. 
Используем ThunkCreator что бы настроить Фанк с помощью замыкания `const addPostThunkCreator=(message)=>(dispatch)=>{.....`
Саму Thunk тоже можно диспатчить. Для диспатча Thunk нужен `Мидлвэар` MW. Где подкючаем?! В коде есть.

UI-BLL-DAL-server при такой схеме, мы не можем делать запрос в редюсерах, это сайд эфект, а редюсер чистая функция. Поэтому thunk.

HOC (1.16.37) На вход получает компоненту, а возвращает другую, контейнерную компоненту

Redux-form (1.17.19) библиотека у которой есть альтернатива называемая Formik

Selectors (1.19.) (81 урок)  промежуточный слой, промежуточные функции.
